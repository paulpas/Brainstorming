...
```
# vCluster API System Architecture Diagram

```mermaid
flowchart TB
    subgraph "EKS Cluster"
        subgraph "vCluster API Service"
            GinAPI[Gin API Service]
            AuthModule[Authentication Module]
            ConfigModule[Configuration Module]
            DynEndpoints[Dynamic Endpoints]
            PruneService[Pruning Service]
            ...
            AuthModule --> JWT[JWT Auth]
            AuthModule --> APIKey[API Key Auth]

            ConfigModule --> YAMLConfig[YAML Config]
            ConfigModule --> EnvVars[Environment Variables]

            DynEndpoints --> CmdExec[Command Execution]

            PruneService --> Scheduler[Cluster Pruning]
        end

        subgraph "vCluster Resources"
            VClusters[Virtual Clusters]
            K8sNamespaces[Kubernetes Namespaces]
            VClusters --> K8sNamespaces
        end

        subgraph "Handlers"
            ClusterHandlers[Cluster Handlers]
            WebhookHandlers[Webhook Handlers]
            DynamicHandlers[Dynamic Endpoint Handlers]

            ClusterHandlers --> ClusterService[Cluster Service]
            WebhookHandlers --> GitHubHook[GitHub Webhook]
            WebhookHandlers --> BitbucketHook[Bitbucket Webhook]
            DynamicHandlers --> DynCmdExec[Dynamic Command Execution]
        end

        GinAPI --> ClusterHandlers
        GinAPI --> WebhookHandlers
        GinAPI --> DynamicHandlers

        ClusterService --> VCCli[vCluster CLI]
        ClusterService --> K8sCli[Kubectl CLI]

        VCCli -- "create/connect/delete" --> VClusters
        K8sCli -- "manage" --> K8sNamespaces

        PruneService -- "monitor/clean" --> VClusters
    end

    Client[Client Application] -- "REST API" --> GinAPI
    GitHubWebhook[GitHub Webhooks] -- "POST /webhooks/github" --> WebhookHandlers
    BitbucketWebhook[Bitbucket Webhooks] -- "POST /webhooks/bitbucket" --> WebhookHandlers
    ConfigFiles[Config Files] -- "load on startup" --> ConfigModule
    EndpointConfig[Dynamic Endpoints Config] -- "define custom endpoints" --> DynEndpoints
...
sequenceDiagram
    ...
    participant GinAPI as Gin API
    participant AuthMW as Auth Middleware
    participant Handler as Route Handler
    participant SVC as Cluster Service
    participant CommandExec as Command Executor
    participant vClusterCLI as vCluster CLI

    Client->>GinAPI: HTTP Request
    Note over GinAPI: Route matching
    GinAPI->>AuthMW: Process request

    alt Authentication Required
        AuthMW->>AuthMW: Extract credentials

        alt JWT Authentication
            AuthMW->>AuthMW: Validate JWT token
        else API Key Authentication
            AuthMW->>AuthMW: Verify API key
        end

        alt Authentication Success
            AuthMW->>Handler: Call route handler
        else Authentication Failure
            AuthMW-->>Client: 401 Unauthorized
        end
    else No Authentication Required
        GinAPI->>Handler: Call route handler directly
    end

    Handler->>SVC: Call service function
    SVC->>CommandExec: Execute vCluster command
    CommandExec->>vClusterCLI: Run vCluster CLI with args
    vClusterCLI-->>CommandExec: Command output
    CommandExec-->>SVC: Process result
    SVC-->>Handler: Return processed data
    Handler-->>Client: HTTP Response
```

## Cluster Lifecycle Management Diagram

```mermaid
stateDiagram-v2
    [*] --> ClusterRequested: POST /api/v1/clusters

    ClusterRequested --> Validating: Validate request
    Validating --> Failed: Invalid request
    Validating --> Creating: Valid request

    Creating --> Failed: Creation error
    Creating --> Running: Cluster created successfully

    Running --> Connecting: GET /api/v1/clusters/:name
    Connecting --> Running: Return connection details

    Running --> Monitoring: Pruning service
    Monitoring --> Expired: TTL exceeded
    Monitoring --> Running: TTL valid

    Running --> Deleting: DELETE /api/v1/clusters/:name
    Expired --> Deleting: Auto-cleanup
...
flowchart TB
    subgraph "Configuration System"
        ConfigFiles[Config Files] --> ConfigLoader[Config Loader]
        EnvVars[Environment Variables] --> ConfigLoader
        ConfigLoader --> ConfigInstance[Config Singleton]
        ConfigInstance -- "Provide settings" --> Components
    end

    subgraph "Component Dependencies"
        Components[All Components]
        Components --> AuthSystem[Authentication System]
        Components --> ClusterMgmt[Cluster Management]
        Components --> DynamicEndpoints[Dynamic Endpoints]
        Components --> WebhookProcessing[Webhook Processing]
    end

    subgraph "External Dependencies"
        VClusterCLI[vCluster CLI] --> ClusterMgmt
        Kubectl[Kubectl] --> ClusterMgmt
        ExtCmds[External Commands] --> DynamicEndpoints
    end

    subgraph "Data Flow"
        Client[Client Request] --> RESTEndpoints[REST API Endpoints]
        RESTEndpoints --> AuthSystem
        AuthSystem --> ClusterMgmt
        ClusterMgmt --> K8sCluster[Kubernetes Cluster]
        K8sCluster --> Response[Response Data]
        Response --> Client
    end
```
...
flowchart TB
    ...
    subgraph "Authentication Methods"
        JWT[JWT Tokens] --> TokenVerify[Token Verification]
        TokenVerify --> Claims[Token Claims]

        APIKey[API Key] --> KeyLookup[API Key Lookup]
        KeyLookup --> ClientID[Client Identity]
    end

    subgraph "Request Flow"
        Request[Incoming Request] --> AuthHeader[Auth Headers]
        AuthHeader --> Middleware[Auth Middleware]
        Middleware --> Validation[Credential Validation]

        Validation -- "Valid" --> RouteHandler[Route Handler]
        Validation -- "Invalid" --> Rejection[401 Unauthorized]
    end

    subgraph "K8s RBAC Integration"
    ...
    Client[Client] -- "HTTPS Request" --> Request
    RouteHandler -- "Authenticated Command" --> vClusterCLI[vCluster CLI]
    vClusterCLI -- "K8s API Calls" --> EKS[EKS Cluster]
```

## Pruning Service Workflow

```mermaid
flowchart TB
    Start[Start Pruning Service] --> Schedule[Schedule Recurring Job]
    Schedule --> InitialRun[Run Initial Pruning]
    InitialRun --> Wait[Wait for Next Interval]
    Wait --> FetchClusters[Fetch All Clusters]

    FetchClusters --> IterateClusters{For Each Cluster}

    IterateClusters --> CheckTTL{Has TTL?}
    CheckTTL -- No --> NextCluster[Next Cluster]
    CheckTTL -- Yes --> ParseTTL[Parse TTL Value]

    ParseTTL --> CalculateExpiry[Calculate Expiry Time]
    CalculateExpiry --> CheckExpired{Is Expired?}

    CheckExpired -- No --> LogTimeLeft[Log Time Remaining]
    CheckExpired -- Yes --> DeleteCluster[Delete Cluster]

    LogTimeLeft --> NextCluster
    DeleteCluster --> NextCluster

    NextCluster -- More Clusters --> IterateClusters
    NextCluster -- No More Clusters --> LogSummary[Log Summary]

    LogSummary --> Wait
```

## Dynamic Endpoints Configuration Flow

```mermaid
flowchart LR
    subgraph "Dynamic Endpoint Configuration"
        Config[YAML Config File] --> Parser[YAML Parser]
        Parser --> Validation[Validate Endpoints]
        Validation --> EndpointRegistry[Register Endpoints]
    end

    subgraph "Request Processing"
        Request[Client Request] --> Router[Gin Router]
        Router --> Matcher[Route Matcher]
        Matcher -- "Dynamic Route" --> DynamicHandler[Dynamic Handler]
        Matcher -- "Standard Route" --> StaticHandler[Static Handler]

        DynamicHandler --> ParamSubstitution[Parameter Substitution]
        ParamSubstitution --> CommandBuilder[Command Builder]
        CommandBuilder --> CommandExec[Execute Command]
        CommandExec --> ResultParser[Parse Result]
        ResultParser --> Response[HTTP Response]
    end

    EndpointRegistry -- "Register routes" --> Router

    subgraph "Example Dynamic Endpoint"
        EndpointDefinition["
        path: /tools/pod-logs/:namespace/:pod
        method: GET
        command: kubectl
        args:
          - logs
    ...
    EndpointDefinition -- "defines" --> EndpointRegistry
```
